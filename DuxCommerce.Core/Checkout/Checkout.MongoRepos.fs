    ...
    let getCheckout :GetCheckout = 
        fun shopperId ->
            let get (db:IMongoDatabase) =
                let checkouts = db.GetCollection<CheckoutDto>(CollectionName.Checkout)
                let checkoutDto = checkouts.Find(fun c -> c.ShopperId = shopperId).FirstOrDefault()
                match box checkoutDto with
                | null -> 
                    let newCheckout = {CheckoutDto.create with ShopperId = shopperId}
    ...
    let saveCheckout :SaveCheckout = 
        fun dto ->
            let save (db:IMongoDatabase) = 
                let checkouts = db.GetCollection<CheckoutDto>(CollectionName.Checkout)
                checkouts.ReplaceOne((fun x -> x.Id = dto.Id), dto) |> ignore
