...
module internal CartItem =
    ...
    let addQtyIf (cmd: AddCartItemCommand) cartItem =
        let addQty cartItem quantity =
            let newQuantity =
                ItemQuantity.add cartItem.Quantity quantity

            let newTotal =
                ItemTotal.calculate cartItem.Price newQuantity

            { cartItem with
                  Quantity = newQuantity
                  ItemTotal = newTotal }

        if cartItem.ProductId = cmd.ProductId then
            addQty cartItem cmd.Quantity
        else
            cartItem
    ...
    let update (cmds: UpdateCartItemCommand seq) cartItem =
        let updateQty cartItem quantity =
            let newTotal =
                ItemTotal.calculate cartItem.Price quantity
        ...
        let updateQtyIf cartItem (itemCmd: UpdateCartItemCommand) =
            if itemCmd.ProductId = cartItem.ProductId then
                seq { updateQty cartItem itemCmd.Quantity }
            else
                Seq.empty
...
module ShoppingCart =
    ...
    let addCartItem cart product (cmd: AddCartItemCommand) =
        let lineItems =
            ...
            | Some _ -> cart.LineItems |> Seq.map (CartItem.addQtyIf cmd)
            | None ->
