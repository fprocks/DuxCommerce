...
module UnitSystem =
    ...
    let create system =
        ...
        | _ ->
            let msg =
                "UnitSystem must be 'ImperialSystem' or 'MetricSystem'"
...
module ShippingMethodType =
    ...
    let create methodType =
        match methodType with
        | "ByWeight" -> Ok ByWeight
        | "ByQuantity" -> Ok ByQuantity
        | "ByOrderTotal" -> Ok ByOrderTotal
        | _ ->
            let msg =
                "ShippingRateType must be one of 'ByWeight', 'ByQuantity', 'ByOrderTotal'"
