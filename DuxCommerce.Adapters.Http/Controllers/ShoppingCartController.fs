...
type ShoppingCartController (logger : ILogger<ShoppingCartController>) =
    ...
    member this.Post(shopperId: string, request: AddCartItemRequest) : IActionResult =
        let result = ConfigReader.execute (ShoppingCartUseCases.addCartItem shopperId request)
        match result with
        | Ok c -> base.Ok(c) :> _
    ...
    member this.Put(shopperId: string, request: UpdateCartRequest) : IActionResult =
        let result = ConfigReader.execute (ShoppingCartUseCases.updateCart shopperId request)
        match result with
        | Ok c -> base.Ok(c) :> _
        | Error m -> base.Convert(m)
    ...
    member this.Delete(shopperId: string, request: DeleteCartItemRequest) : IActionResult =
        let result = ConfigReader.execute (ShoppingCartUseCases.deleteCartItem shopperId request)
        match result with
        | Ok c -> base.Ok(c) :> _
        | Error m -> base.Convert(m)
